...
const MetricCard: React.FC<MetricCardProps> = ({
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-lg flex items-center space-x-4">
      <div
        className={`p-4 rounded-full ${subLabel ? 'bg-orange-100 text-orange-500' : 'bg-green-100 text-green-500'}`}
      >
        {icon}
      </div>
      <div>
...
const PlutoDashboard: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 font-sans text-gray-800">
      {/* Sidebar */}
      <aside
        className={`bg-gray-800 text-white w-64 space-y-6 py-7 px-2 fixed inset-y-0 left-0 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition duration-200 ease-in-out z-50`}
      >
        {/* Header Sidebar */}
        ...
        <nav>
          <Link
            href="#"
            className="flex items-center space-x-2 py-2 px-4 rounded-lg bg-orange-600 text-white"
          >
            <FaTachometerAlt />
            <span>Dashboard</span>
          </Link>
          <Link
            href="#"
            className="flex items-center space-x-2 py-2 px-4 text-gray-300 hover:bg-gray-700 rounded-lg mt-2"
          >
            <FaCubes />
            <span>Widgets</span>
          </Link>
